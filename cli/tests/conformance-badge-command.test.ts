...
const passingReport = {
  ...
  package: { name: 'a2a-warp', version: '1.0.0' },
  protocolVersion: '1.0',
  endpoint: {
    url: 'http://agent.test',
...
describe('conformance-badge command', () => {
  ...
  it('generates an SVG badge file from a valid report', async () => {
    ...
    await runCli(['node', 'a2a-warp', 'conformance-badge', reportPath, '--output', badgePath]);
  ...
  it('outputs a Markdown image reference with --markdown', async () => {
    ...
    await runCli(['node', 'a2a-warp', 'conformance-badge', reportPath, '--markdown']);

    expect(stdout).toContain('![A2A Warp Conformance](badge.svg)');
  });
  ...
  it('fails on missing report file', async () => {
    ...
    await runCli(['node', 'a2a-warp', 'conformance-badge', '/nonexistent/report.json']);
  ...
  it('fails on invalid JSON report', async () => {
    ...
    await runCli(['node', 'a2a-warp', 'conformance-badge', reportPath]);
