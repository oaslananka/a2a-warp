...
describe('CLI command module structure', () => {
  it('keeps the root entrypoint focused on program construction and registration', () => {
    const source = readFileSync(resolve(repoRoot, 'cli/src/index.ts'), 'utf8');

    expect(source.split('\n').length).toBeLessThanOrEqual(150);
    expect(source).not.toContain("from '@oaslananka/a2a-warp'");
    expect(source).not.toContain("from '@oaslananka/a2a-warp-registry'");
  ...
  it('has a module and dedicated test file for each CLI command refactor target', () => {
    for (const command of commandModules) {
      expect(existsSync(resolve(repoRoot, `cli/src/commands/${command}.ts`))).toBe(true);
    }

    for (const testFile of dedicatedCommandTests) {
      expect(existsSync(resolve(repoRoot, `cli/tests/${testFile}`))).toBe(true);
    }
  });
});
