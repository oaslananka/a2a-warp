  ...
  examples: [
    {
      ...
      bash: ['a2a-warp release-check'],
      powershell: ['a2a-warp release-check'],
    },
    {
      title: 'Emit machine-readable JSON report.',
      bash: ['a2a-warp release-check --json'],
      powershell: ['a2a-warp release-check --json'],
    },
  ],
  ...
  return applyCommandDoc(new Command('release-check'), releaseCheckCommandDoc).action(() => {
...

    {
      const schemaBuild = runPnpm(
        'Schema core build',
        ['--filter', '@oaslananka/a2a-warp', 'run', 'build'],
        workspaceRoot,
      );
