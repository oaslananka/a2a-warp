  ...
  examples: [
    {
      ...
      bash: ['a2a-warp conformance-badge report.json --output badge.svg'],
      powershell: ['a2a-warp conformance-badge report.json --output badge.svg'],
    },
    {
      title: 'Print a Markdown image reference to stdout.',
      bash: ['a2a-warp conformance-badge report.json --markdown'],
      powershell: ['a2a-warp conformance-badge report.json --markdown'],
    },
  ],
  ...
  return command.action(
    async (reportFile: string, commandOptions: ConformanceBadgeCommandOptions) => {
      ...
      if (commandOptions.markdown) {
        emitResult(`![A2A Warp Conformance](badge.svg)`, options);
        return;
      }
