...
function renderCard(name: string): string {
  return `{
      protocolVersion: '1.0',
      name: '${name}',
      description: 'A2A agent scaffolded with A2A Warp',
...
`;
  ...
  if (options.adapter === 'langchain') {
    return `import { LangChainAdapter } from '@oaslananka/a2a-warp-adapters';
import type { AgentCard } from '@oaslananka/a2a-warp';
...
`;
  ...
  return `import { BaseAdapter } from '@oaslananka/a2a-warp-adapters';
import { logger, type Artifact, type Message, type Task } from '@oaslananka/a2a-warp';
...
function renderIndexSource(name: string, options: ScaffoldOptions): string {
  if (options.adapter === 'pack-research-team') {
    return `import { RegistryServer } from '@oaslananka/a2a-warp-registry';
...
Scaffolded with \`a2a-warp scaffold\`.
