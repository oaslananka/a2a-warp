...
A2A Warp treats trace span names, metric names, and metric labels as a stable operational surface. Changes to the names or required attributes below must update the telemetry snapshot tests intentionally.

OpenTelemetry trace and metric instruments use the instrumentation scope `@oaslananka/a2a-warp` with version `1.0.0`. Prometheus text endpoints remain available for local runtime and registry scraping.
...
```bash
pnpm --filter @oaslananka/a2a-warp run test -- telemetry
pnpm --filter @oaslananka/a2a-warp-registry run test -- metrics
```
