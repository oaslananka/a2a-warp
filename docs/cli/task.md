...
| Command                | Summary                                 |
| ---------------------- | --------------------------------------- |
| `a2a-warp task send`   | Send a text message to an A2A endpoint. |
| `a2a-warp task stream` | Stream events for a sent task message.  |
| `a2a-warp task status` | Fetch status for an existing task.      |
| `a2a-warp task cancel` | Cancel an existing task.                |
...
```bash
a2a-warp task send http://127.0.0.1:3000 "hello"
```
