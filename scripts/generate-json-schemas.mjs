...
const publicSchemasModule = await import(
  pathToFileURL(resolve(repoRoot, 'packages/core/dist/schemas/public.js')).href
);

const definitions = publicSchemasModule.publicJsonSchemaDefinitions;
if (!Array.isArray(definitions) || definitions.length === 0) {
  throw new Error('packages/core/src/schemas/public.ts must export publicJsonSchemaDefinitions');
}
