...
const safeToPublish =
  repository === 'oaslananka/a2a-warp' &&
...
const state = {
  ...
  blockers: [
    ...
    ...(repository === 'oaslananka/a2a-warp'
      ? []
      : ['Canonical release repository is oaslananka/a2a-warp']),
  ],
};
...
function getRemoteRepository() {
  ...
  try {
    ...
    return 'oaslananka/a2a-warp';
  }
  const match = url.match(/github\.com[:/](?<owner>[^/]+)\/(?<repo>[^/.]+)(?:\.git)?$/);
  return match?.groups ? `${match.groups.owner}/${match.groups.repo}` : 'oaslananka/a2a-warp';
}
