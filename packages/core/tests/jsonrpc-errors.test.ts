...
describe('documentation URL configuration', () => {
  ...
  it('normalizes the configured public docs URL', async () => {
    vi.stubEnv('A2A_WARP_DOCS_PUBLIC_URL', 'https://docs.example.com/base');
    vi.resetModules();
  ...
  it('uses the default docs URL when the environment value is blank', async () => {
    vi.stubEnv('A2A_WARP_DOCS_PUBLIC_URL', '   ');
    vi.resetModules();
    const docs = await import('../src/config/docs.js');

    expect(docs.getDocsUrl()).toBe('https://docs.a2a-warp.local/');
    expect(docs.getDocsUrl('quickstart')).toBe('https://docs.a2a-warp.local/quickstart');
  });

  it('keeps the statically imported default URL stable for current module users', () => {
    expect(getDocsUrl('security/auth')).toBe('https://docs.a2a-warp.local/security/auth');
  });
});
