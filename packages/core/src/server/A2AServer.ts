...
} from '../security/AgentCardSigner.js';
...
export abstract class A2AServer {
  ...
  private setupMiddleware() {
    ...
    const rateLimiter = createRateLimiter(this.options.rateLimit ?? {}, this.rateLimitStore);
  ...
  private setupRoutes() {
    const serveCard = async (_req: Request, res: Response) => {
      const card = this.options.signingKey
        ? await signAgentCard(this.agentCard, this.options.signingKey)
        : this.agentCard;
      res.json(card);
    };
    ...
    try {
      ...
      switch (req.method) {
        ...
        case 'tasks/pushNotification/set': {
          ...
          if (typeof taskId !== 'string' || typeof rawPushNotificationConfig !== 'object') {
            throw new JsonRpcError(
          ...
          const pushNotificationConfig = await this.normalizePushNotificationConfig(
            rawPushNotificationConfig as NonNullable<
              NonNullable<MessageSendParams['configuration']>['pushNotificationConfig']
            >,
          );
          const config = this.taskManager.setPushNotification(taskId, pushNotificationConfig);
          return config;
        }
        ...
        case 'tasks/pushNotification/get': {
          ...
          return this.taskManager.getPushNotification(taskId) ?? null;
        }
  ...
  private async handleMessageRequest(
    ...
    this.processTaskInternal(task, message, signal).catch((error) => {
  ...
  protected async processTaskInternal(
    ...
    const span = a2aWarpTracer.startSpan('a2a.processTask', {
      attributes: {
        'a2a.task_id': task.id,
