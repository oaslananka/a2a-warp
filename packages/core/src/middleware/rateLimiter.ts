...
/**
 * Adapts the A2A Warp rate-limit store to express-rate-limit.
 *
...
class ExpressRateLimitStoreAdapter implements ExpressRateLimitStore {
  ...
  decrement(_key: string): void {
    // The A2A Warp store contract is monotonic within a window.
  }

  resetKey(_key: string): void {
    // Per-key reset is not part of the public A2A Warp store contract.
  }
