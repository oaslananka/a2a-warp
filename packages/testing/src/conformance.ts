...
const conformanceMessages = {
  '1.0': {
    ...
    configuration: {
      ...
      extensions: [
        {
          uri: 'https://a2a-warp.test/extensions/conformance/v1',
          version: '1.0.0',
          required: false,
        },
      ],
...
export async function runConformanceSuite({
  ...
  cases.push(
    await runCase('protocol-version', 'Verify requested protocol support', true, async () => {
      ...
      if (!supportsProtocolVersion(agentCard, protocolVersion)) {
        throw new Error(`Endpoint does not advertise protocol ${protocolVersion}`);
      }
      return { requestedProtocolVersion: protocolVersion };
    }),
  );
  ...
  cases.push(
    await runCase('message-send', 'Run the message/send conformance fixture', true, async () => {
      ...
      const task = await client.sendMessage(createConformanceMessageParams(protocolVersion));
