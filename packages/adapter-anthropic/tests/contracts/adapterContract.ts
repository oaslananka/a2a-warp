...
import type { BaseAdapter } from '@oaslananka/a2a-warp-adapter-base';
  ...
  describe(`${spec.adapterName} adapter contract`, () => {
    ...
    it(`${spec.adapterName}: completes task state and stores artifacts through A2A lifecycle`, async () => {
      ...
      try {
        ...
        expect(submitted.status.state).toBe('WORKING');

        const completed = await waitForTaskState(instance.adapter, submitted.id, 'COMPLETED');
