...
This page mirrors the repository compatibility policy for A2A Warp runtime
versions, package surfaces, protocol fixtures, transports, optional peers, and
deprecation windows.
...
the repository runtime manifest.

## Runtime Compatibility

The workspace engine range is Node.js `>=22.22.1 <25` and pnpm `>=11 <12`.
Development tooling pins pnpm `11.2.2` through `packageManager` and
...
| pnpm 11            | Supported package manager | `11.2.2`                                  | Latest registry metadata checked separately                   | Required for workspace scripts and lockfile consistency.                       |
...
| ----------------------------------- | --------------- | --------------- | ---------------------------------------------------------- |
| `@oaslananka/a2a-warp`              | `1.0.3`         | `>=22.22.1 <25` | Core runtime, client APIs, auth, telemetry, and storage.   |
| `@oaslananka/a2a-warp-client`       | `1.0.3`         | `>=22.22.1 <25` | Standalone client re-exports for smaller installs.         |
| `@oaslananka/a2a-warp-adapters`     | `1.0.3`         | `>=22.22.1 <25` | Optional provider and framework adapters.                  |
| `@oaslananka/a2a-warp-registry`     | `1.0.3`         | `>=22.22.1 <25` | Registry server, discovery, health, and storage helpers.   |
| `@oaslananka/a2a-warp-cli`          | `1.0.3`         | `>=22.22.1 <25` | Published `a2a-warp` command-line interface.               |
| `create-a2a-warp`                   | `1.0.3`         | `>=22.22.1 <25` | Project scaffolder.                                        |
| `@oaslananka/a2a-warp-mcp-bridge`   | `1.0.3`         | `>=22.22.1 <25` | A2A and MCP mapping helpers.                               |
| `@oaslananka/a2a-warp-ws`           | `1.0.3`         | `>=22.22.1 <25` | WebSocket transport helpers.                               |
| `@oaslananka/a2a-warp-grpc`         | `1.0.3`         | `>=22.22.1 <25` | gRPC transport helpers.                                    |
| `@oaslananka/a2a-warp-testing`      | `1.0.3`         | `>=22.22.1 <25` | Fixtures, matchers, local server, and conformance helpers. |
| `@oaslananka/a2a-warp-codex-bridge` | `1.0.3`         | `>=22.22.1 <25` | Codex-style tool bridge helpers.                           |
...
| `@oaslananka/a2a-warp-adapters` | `@anthropic-ai/sdk`  | `^0.39.0 \|\| ^0.95.0 \|\| ^0.96.0` |
